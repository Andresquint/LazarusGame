/*
This class extends GameControl. It loads the
...
public class LazarusControl extends GameControl {
    ...
    private ArrayList<DataField> box_array = new ArrayList<>();
    private ArrayList<ButtonControl> stop_array = new ArrayList<>();
    ...
    public void init() {
        ...
        try {
            ...
            box_array.add(new DataField(BoxKind[rand], lazarus_array.get(0).getX(), 0, speed, (char)(rand + 49), true));
            eventNotifier = new EventObserver();
            eventNotifier.addObserver(lazarus_array.get(0));
            KeyControl key = new KeyControl();
            addKeyListener(key);
        } catch (Exception e) {
            System.out.println("Error - It is not file name for sprite: " + e);
        }
    }
    ...
    public void selectRandomBox() {
        BufferedImage[][] BoxKind = {CardSprt, WoodSprt, StoneSprt, MetalSprt};
        int rand = ThreadLocalRandom.current().nextInt(0, 4);        
        box_array.add(new DataField(BoxKind[rand], 0, 440, speed, (char)(rand + 49), false));
    }
            ...
            for (int j = 0; j < box_array.size(); j++) {
                if (j != i && box_array.get(j).getIsDisplayed()) {
                    if (box_array.get(i).isDropping()) {
                        if (box_array.get(j).getKindOfBox() > box_array.get(i).getKindOfBox() || box_array.get(j).getKindOfBox() == '0' ||
                                box_array.get(i).getKindOfBox() == box_array.get(j).getKindOfBox()) {
            ...
            while (a > -1) {
                while (a != '\n') {
                    if (a == ' ') {
                       ...
                       stop_array.add(new ButtonControl(StopSprt, x, y));
                       x += pixelX; 
                    }
                    else if(a == 'x'){
                        box_array.add(new DataField(WallSprt, x, y, speed, '0', false));
                        x += pixelX; 
                    }
                    else if(a == 'l'){
    ...
    public void draw() {
        ...
        if (level > 3) {   // game won         
           ...
           g2.drawString("\t\tYou are Alive!", 400, 400);
